...
router.get("/new",middleware.isLoggedIn,(req,res)=>{
    social.findById(req.params.id,(err,social)=>{
        ...
        else{res.render("comments/new",{social:social});}})
});
/////////////////////////////////////////////////////// Adding Comment
...
router.get("/:comment_id/edit",middleware.checkCommentOwnership,(req,res)=>{
    social.findById(req.params.id,(err,foundsocial)=>{
        if(err||!foundsocial){req.flash("error","Sorry! Cannot Find It ! Please Delete It And Add A New One");
            console.log(err);
            return res.redirect("back");}
        Comment.findById(req.params.comment_id,(err,foundComment)=>{
            if(err){res.redirect("back");}
            else{res.render("comments/edit",{social_id:req.params.id,comment:foundComment});}});});
});
///////////////////////////////////////////////////////////////////////////  Updating Comment
router.put("/:comment_id",middleware.checkCommentOwnership,(req,res)=>{
Comment.findByIdAndUpdate(req.params.comment_id,req.body.comment,(err)=>{
if(err){res.redirect("back");}
else{res.redirect("/socials/"+req.params.id);}});
});
////////////////////////////////////////////////////////////////////////////// Delete Comment
router.delete("/:comment_id",middleware.checkCommentOwnership ,(req,res)=>{
    Comment.findByIdAndDelete(req.params.comment_id,(err,deleted)=>{
        if(err){res.redirect("back");}
        else{req.flash("success","Comment Deleted ! PLease Refresh If Not Visible");
